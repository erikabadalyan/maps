...
def map_and_filter(s, map_fn, filter_fn):
    ...
    """
    # BEGIN Question 0
    return [map_fn(values) for values in s if filter_fn(values)]
    # END Question 0
...
def zip(*sequences):
    ...
    ['c', 3, 'mi']
    """
    return [[sequence[i] for sequence in sequences] for i in range(0, len(sequences[0]))]
