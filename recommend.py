...
def group_by_centroid(restaurants, centroids):
    ...
    for r in restaurants:
        pair = (find_closest(restaurant_location(r), centroids), r)
        tuplelist.append(pair)
    return group_by_first(tuplelist)
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        # BEGIN Question 6
        "*** YOUR CODE HERE ***"
        newclusters = group_by_centroid(restaurants, centroids)
        centroids = [find_centroid(newcluster) for newcluster in newclusters]
        # END Question 6
        n += 1
    return centroids
...
def best_predictor(user, restaurants, feature_fns):
    ...
    predictor_best, r_sq = max(predictor_list, key = lambda x: x[1])
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        if restaurant in reviewed:
            dictionary_rating[restaurant_name(restaurant)] = user_rating(user, restaurant_name(restaurant))
        else:
            dictionary_rating[restaurant_name(restaurant)] = predictor(restaurant)
    return dictionary_rating
    # END Question 9
